...
export class HayvanKayitService {
  ...
  getHayvanKayitsByHayvanTedavi(
    hayvanKayitId: number
  ): Observable<ListResponseModel<HayvanKayit>> {
    let newPath =
      this.apiUrl + 'hayvankayits/getbyid?hayvanKayitId=' + hayvanKayitId;
    return this.httpClient.get<ListResponseModel<HayvanKayit>>(newPath);
  }
  add(hayvanKayit: HayvanKayit): Observable<ResponseModel> {
    return this.httpClient.post<ResponseModel>(
      this.apiUrl + 'hayvanKayits/add',
      hayvanKayit
    );
  }
}
